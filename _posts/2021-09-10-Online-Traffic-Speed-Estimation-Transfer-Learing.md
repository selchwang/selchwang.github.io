...
数据集采样操作：随机移除一些道路的数据。从$\mathcal{V}^+_t$中采样M次，
得到$\mathcal{V}^-_{t,m}$，其中m是数据集的序号索引。

随后用预训练数据对生成器、鉴别器进行训练。每次迭代中，输入$\mathcal{V}^-_{t,m}$到生成器，
得到$\hat  {\mathcal{V}}^-_{t,m}$，作为对$\mathcal{V}^+_t$的一次估计。
